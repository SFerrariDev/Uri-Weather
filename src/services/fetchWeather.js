    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (response.status === 400) {
                errorMessage = "Las entradas proporcionadas como parámetros no eran válidas para la solicitud.";
...
export async function getWeather(search, setLoading, handleError) {
    const weatherApiData = await fetchWeatherData('realtime', search, setLoading, handleError);
...
export async function getForecast(search, setLoading, handleError) {
    const forecastApiData = await fetchWeatherData('forecast', search, setLoading, handleError);
